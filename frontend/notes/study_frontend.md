...
### 8. 주요 웹 캡처 라이브러리별 차이 및 선택 기준

| 라이브러리        | 주요 특징/장점                                                                    | 단점/제약                                                                                  | 비고/추천 상황                  |
| ----------------- | --------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ | ------------------------------- |
| **html2canvas**   | - 가장 널리 쓰임<br>- CSS 스타일, 이미지, 폰트 등 대부분 지원<br>- 사용법 단순    | - SVG, 필터, 일부 CSS3 미지원<br>- 외부 이미지 CORS 이슈<br>- 실제 렌더 결과와 100% 일치 X | - 일반적인 UI 캡처에 적합       |
| **dom-to-image**  | - SVG 기반 렌더링<br>- 외부 이미지, 폰트 등 지원<br>- base64, blob 등 다양한 포맷 | - 일부 CSS3, 필터, 폰트 미지원<br>- 복잡한 레이아웃에서 깨질 수 있음                       | - SVG/벡터 기반 캡처 필요시     |
| **rasterizeHTML** | - HTML+CSS를 SVG로 변환 후 캡처<br>- 외부 CSS, 폰트, 이미지 지원                  | - 최신 CSS3, JS 동적 렌더링 미지원<br>- 복잡한 SPA에는 부적합                              | - 정적 HTML, 문서 캡처에 적합   |
| **puppeteer**     | - Headless 브라우저로 실제 렌더링 결과를 캡처<br>- PDF, 전체 페이지 등 지원       | - 서버/Node 환경 필요<br>- 클라이언트 JS에서 직접 사용 불가                                | - 서버 사이드, 고품질 캡처 필요 |

#### (1) html2canvas

- 가장 널리 쓰이는 프론트엔드 캡처 라이브러리
- 대부분의 CSS, 이미지, 폰트, 배경 등 지원하지만, 일부 최신 CSS3, SVG, 필터 등은
  미지원
- 외부 이미지(CORS) 이슈가 있을 수 있음
- 실제 렌더 결과와 100% 일치하지 않을 수 있음(특히 복잡한 레이아웃)

#### (2) dom-to-image

- SVG 기반으로 DOM을 이미지로 변환
- base64, blob, SVG 등 다양한 포맷 지원
- 일부 CSS3, 폰트, 필터 등은 미지원, 복잡한 레이아웃에서 깨질 수 있음

#### (3) rasterizeHTML

- HTML+CSS를 SVG로 변환 후 캡처
- 외부 CSS, 폰트, 이미지 등 지원
- 최신 CSS3, JS 동적 렌더링 미지원, SPA에는 부적합

#### (4) puppeteer

- Headless Chrome 등 브라우저 엔진을 이용해 실제 렌더링 결과를 캡처
- PDF, 전체 페이지, 고해상도 등 지원
- 서버/Node 환경 필요, 클라이언트 JS에서는 직접 사용 불가

#### (5) 기타

- **html-to-image**: dom-to-image의 개선판, SVG/폰트 지원 강화, 사용법 유사
- **canvg**: SVG만 캡처, SVG to PNG 변환에 특화

### [실무 선택 기준]

- **일반적인 UI/썸네일 캡처**: html2canvas, dom-to-image, html-to-image
- **SVG/벡터 기반, 외부 폰트/이미지**: dom-to-image, html-to-image
- **정적 문서/HTML**: rasterizeHTML
- **서버 사이드, 고품질/전체 페이지**: puppeteer
- **복잡한 CSS3, 동적 렌더링**: puppeteer(서버), 아니면 Portal+렌더링 타이밍 보
  장 필수
