	...
	plugins: [
		...
		VitePWA({
			...
			manifest: {
				...
				start_url: '/', // 앱 시작 경로
				scope: '/', // PWA 적용 범위
				// @ts-ignore - FCM Sender ID for push notifications
				gcm_sender_id: import.meta.env.VITE_FIREBASE_MESSAGING_SENDER_ID, // 푸시 알림 보내는 사람의 ID
				icons: [
					{
			...
			workbox: {
				disableDevLogs: true,
				clientsClaim: true, // 새 서비스워커가 즉시 모든 탭을 제어 === 모든 탭에서 FCM 메시지 수신
				skipWaiting: true, // 새 서비스워커가 대기 없이 즉시 활성화 === 모든 탭에서 FCM 메시지 수신
				globPatterns: ['**/*.{js,css,html,woff2,png,jpg,svg,mp4}'], // precache할 파일 확장자 === 모든 파일 캐시
